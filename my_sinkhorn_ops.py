...
def my_sinkhorn(log_alpha, n_iters = 20):
    ...
    """
    # [bz * samples_per_num, n_numbers, n_numbers] 
    n = log_alpha.size()[1] # n_numbers
...
def my_gumbel_sinkhorn(log_alpha, temp=1.0, n_samples=1, noise_factor=1.0, n_iters=20, squeeze=True):
    ...
    n_iters: number of sinkhorn iterations. Should be chosen carefully, in
      inverse correspondence with temp to avoid numerical instabilities. # temp越小，需要迭代的次数越多；
    ...
    # [bz * samples_per_num * n_samples, n_numbers, n_numbers]
    log_alpha_w_noise = log_alpha.repeat(n_samples, 1, 1)  # 为啥又要重复n_numbers个？

    if noise_factor == 0:
        noise = 0.0
    else: # -torch.log(eps - torch.log(U + eps)) * noise_factor
        noise = to_var(my_sample_gumbel([n_samples*batch_size, n, n])*noise_factor)

    log_alpha_w_noise = log_alpha_w_noise + noise.to(log_alpha_w_noise.device)
    ...
    if n_samples > 1 or squeeze is False:
        ...
        sink = torch.transpose(sink, 1, 0) # [batch_size, n_samples, n, n]
        log_alpha_w_noise = log_alpha_w_noise.view(n_samples, batch_size, n, n)
        log_alpha_w_noise = torch.transpose(log_alpha_w_noise, 1, 0) # [batch_size, n_samples, n, n]
...
def my_sample_uniform_and_order(n_lists, n_numbers, prob_inc):
    ...
    return (ordered, random, permutations) # # 函数返回排序后的数值、原始的随机数以及排列索引
